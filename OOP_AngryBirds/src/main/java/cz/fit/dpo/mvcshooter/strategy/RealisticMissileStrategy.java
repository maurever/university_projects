 ...
 */
public class RealisticMissileStrategy implements MissileMovementStrategy {
    ...
    public int getNewY(Missile missile) {
        double time = missile.getTimer() * 0.1;
        return (int) (missile.getInicialY() - missile.getSpeed() * time * Math.sin(missile.getAngle()) + (0.5 * missile.getGravity() * time * time));
    }
}
